<template>
  <div class="auth-split" dir="rtl">
    <aside class="auth-brand-panel" aria-hidden="true">
      ...
      <div class="brand-wordmark">Nifraim<span class="brand-wordmark-tld">.com</span></div>
    </aside>

    <main class="auth-form-panel">
      <div class="form-content">
        <h2 class="form-heading">התחברות</h2>
...
<style scoped>
/* ── Cream palette (matches landing chapters 3 & 5) ── */
.auth-split {
  min-height: 100vh;
  display: flex;
  background: #f5f0eb;
...
/* ── Decorative brand panel — warm cream with animated cobalt/teal orbs ── */
.auth-brand-panel {
  position: relative;
  flex: 0 0 45%;
  overflow: hidden;
...
/* Animated orbs — cobalt/teal/sky tones using multiply so they tint the cream */
.orb {
  position: absolute;
  border-radius: 50%;
  filter: blur(70px);
...
/* Diagonal sweep — soft white shine across the cream */
.sweep {
  position: absolute;
  inset: -20%;
...
.wave--1 { animation: waveDrift 6s ease-in-out infinite; }
.wave--2 { animation: waveDrift 8s ease-in-out infinite reverse; }
.wave--3 { animation: waveDrift 10s ease-in-out infinite; }

@keyframes waveDrift {
  0%, 100% { transform: translateX(0) scaleY(1); }
  50%      { transform: translateX(-18px) scaleY(1.04); }
}
...
.brand-toggle-link {
  ...
  padding: 10px 28px;
  border: 1.5px solid rgba(45, 37, 34, 0.25);
  border-radius: 50px;
  color: #2d2522;
  font-weight: 600;
  font-size: 14px;
...
/* ── Form panel — soft surface tone, quieter than decorative ── */
.auth-form-panel {
  flex: 1 1 auto;
  display: flex;
  align-items: center;
  ...
  position: relative;
  background: #f9f6f2;
  border-inline-start: 1px solid rgba(45, 37, 34, 0.06);
}
