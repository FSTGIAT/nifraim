...
=== בלוק "עובדות לקוח" + תוצאות חיפוש — מקור אמת מוחלט ===
כאשר המשתמש שואל על לקוח ספציפי (לפי ת.ז או שם):

1. **בלוק "=== עובדות לקוח ===" (אם קיים)**: זה המקור הראשון.
...
2. **בלוק "--- תוצאות חיפוש לקוחות ---"**: מציג את הלקוח לפי מקור (פרודוקציה/נפרעים).
   - אם מופיעה כותרת "[פרודוקציה]" עם שורות של הלקוח — הלקוח **כן** בפרודוקציה. אל תכחיש.
   - אם מופיעה כותרת "[נפרעים (...)]" עם שורות של הלקוח — הלקוח **כן** בקובץ הנפרעים הזה.
   - **חובה לסרוק את כל הכותרות** לפני שאתה אומר "רק בנפרעים" או "רק בפרודוקציה".
   - אם הלקוח מופיע גם תחת [פרודוקציה] וגם תחת [נפרעים (...)] — האמת היא "בשניהם" (matched), **לא** "רק ב…".
   - דוגמה אסורה (באג שקרה): המשתמש שאל על אבלין פיפרברג. תוצאות החיפוש הראו אותה גם תחת [פרודוקציה] (10 רשומות) וגם תחת [נפרעים (הראל)] (8 רשומות). תשובה אסורה: "אבלין נמצאת רק בנפרעים". תשובה נכונה: "אבלין נמצאת בשניהם — 10 רשומות בפרודוקציה (הפניקס+הראל) ו-8 רשומות בקובץ הנפרעים של הראל".

3. אם הלקוח אינו ברשימה (לא בטופ 30, לא בתוצאות החיפוש) — אמור "אין לי נתון על לקוח זה בקבצים הזמינים", אל תמציא ערכים.

4. אם המשתמש שואל "האם X בפרודוקציה?" התשובה היא **כן** אם יש לפחות שורה אחת בבלוק [פרודוקציה] עם שמו/ת.ז שלו.
...
=== כלל קריטי: דלתא לפי חברה ≠ דלתא של לקוח בודד ===
כאשר המשתמש שואל "כמה עלתה הצבירה ב-<שם חברה>", "מה השינוי ב-<שם חברה>", או דומה:
- **חובה** לקחת את הערך מבלוק "=== שינויים מצטברים לפי חברה ===". זה הסכום של **כל** הלקוחות באותה חברה.
- **אסור** לקחת ערך מבלוק "10 לקוחות בודדים עם השינוי הגדול ביותר" ולדווח אותו כדלתא של חברה — זה הסכום של **לקוח אחד** בלבד, לא של חברה.
- אם בלוק "שינויים מצטברים לפי חברה" לא מכיל את החברה שביקשו — אמור "אין נתון מצטבר לחברה זו בקובץ הנוכחי", **אל תסכם בעצמך** מתוך רשימת הלקוחות הבודדים.
- דוגמה אסורה: "10 לקוחות בודדים: 50700889 אורי פלד (מור) צבירה +₪2,658,447" — אסור לענות "מור עלתה ב-₪2,658,447". זה הסכום של לקוח אחד; הסכום של מור נמצא בבלוק האחר ויכול להיות שונה לחלוטין.
...
async def stream_chat(
    ...
    if view_context:
        system_prompt += (
            ...
            f"{view_context}\n\n"
            "**גבולות השימוש בבלוק 'המסך הנוכחי' — חוקים מחייבים:**\n"
            "1. בלוק זה מתאר *רק* את המסך שהמשתמש צופה בו עכשיו (לרוב חברה אחת או תקופה אחת). "
            "הוא **לא** מתאר את כל הנתונים שלך — לכל הנתונים המלאים יש להשתמש בבלוקים שלמעלה "
            "(=== קבצי נפרעים שהועלו ===, === קובץ פרודוקציה ===, === עמלות לפי חודש === וכו').\n"
            "2. כשהמשתמש שואל שאלה השוואתית או רחבה ('איזו חברה משלמת הכי הרבה', 'סכם את החברות', "
            "'מי גדל הכי הרבה', 'תן לי סקירה') — **חובה לסרוק את כל הבלוקים** של כל החברות והקבצים, "
            "ולא להגביל את התשובה רק לחברה/תקופה במסך הנוכחי. אסור לכתוב 'יש לי רק חברה אחת' "
            "כשבבלוק '=== קבצי נפרעים שהועלו ===' מופיעות מספר חברות.\n"
            "3. כשהשאלה ספציפית למסך הנוכחי ('מי לא שולם בקובץ הזה', 'מה הסכום במסך', "
            "'הראה לי את הלקוחות בטבלה') — אז בלוק 'המסך הנוכחי' הוא המקור הראשי.\n"
            "4. כשבבלוק 'המסך הנוכחי' חסר נתון השוואתי שמופיע בבלוקים האחרים — קח אותו משם, "
            "אל תגיד 'אין לי נתון'.\n"
            "5. אסור להמציא מספרים שלא מופיעים באף בלוק. אם אינך מוצא ערך — אמור 'אין לי את הנתון הזה'.\n"
        )
    ...
    if attachments:
        ...
        user_blocks.append({
            ...
            "text": (
                ...
                "2. **רכיבי עמלה — חוק הסיכום (קריטי)**:\n"
                "   הסכמי עמלות ביטוח בישראל (הפניקס, הראל, וחברות דומות) מציגים את שיעור הנפרעים הסופי כסכום של שני רכיבים:\n"
                "   • **עמלת ספר** — שיעור בסיס לקטגוריה\n"
                "   • **שיעור תגמול** (בהפניקס) או **תוספת נפרעים** (בהראל) — שיעור נוסף\n\n"
                "   **חוק החיבור (חובה — זוהי הדרך הנכונה לחשב את העמלה):**\n"
                "   `שיעור נפרעים סופי = עמלת ספר + שיעור תגמול`\n"
                "   כאשר שני הרכיבים מודפסים מילולית במסמך לאותו מוצר ולאותו טווח שנים.\n\n"
                "   **חוקי אזהרה — אסור לסטות מהם:**\n"
                "   • אם רק עמלת ספר מודפסת לטווח שנים מסוים (ושיעור תגמול ריק/חסר עבור אותו טווח) — דווח רק את עמלת הספר. **אסור להמציא** שיעור תגמול שלא קיים.\n"
                "   • אם רק שיעור תגמול מודפס (ועמלת ספר חסרה) — דווח רק את שיעור התגמול.\n"
                "   • כל מספר שאתה מדווח חייב להופיע מילולית במסמך, או להיות סכום של שני מספרים שמודפסים שניהם בטקסט.\n\n"
                "   **דוגמה תקפה — הפניקס השתלות וטיפולים מיוחדים (test3):**\n"
                "   המסמך מציג: שנה 1-5: ספר 15% (אין תגמול) | שנה 6-15: ספר 15% תגמול 7.2% | שנה 16+: ספר 5% תגמול 7.2%\n"
                "   התשובה הנכונה:\n"
                "     שנה 1-5: 15% (עמלת ספר בלבד)\n"
                "     שנה 6-15: 22.2% (עמלת ספר 15% + שיעור תגמול 7.2%)\n"
                "     שנה 16+: 12.2% (עמלת ספר 5% + שיעור תגמול 7.2%)\n\n"
                "   **דוגמה אסורה (באג שקרה):**\n"
                "   אמירה: 'שנה 1-5: 25.4%' (15 + 10.4) — שגוי. ה-10.4% לא מופיע במסמך לטווח 1-5. אסור להמציא ערך.\n\n"
