    ...
    if attachments:
        ...
        user_blocks.append({
            ...
            "text": (
                ...
                "2. **רכיבי עמלה — קריאה זהירה (חוק קריטי)**:\n"
                "   רוב הסכמי הראל מציגים את העמלה הסופית כסכום של שני רכיבים בטבלאות נפרדות: \n"
                "   • **עמלת ספר** — שיעור בסיס לקטגוריה (חיים, בריאות וכו').\n"
                "   • **תוספת נפרעים** — שיעור נוסף לכל מוצר ספציפי.\n"
                "   • **סה״כ נפרעים** = ספר + תוספת **כאשר שני העמודות מודפסות במפורש במסמך הזה ולמוצר הזה**.\n\n"
                "   **כללים מחייבים — אסור לסטות מהם:**\n"
                "   • אם המסמך מציג עבור מוצר נתון רק עמלת ספר (ללא עמודת תוספת) — דווח רק את עמלת הספר. אסור להוסיף תוספת שאינה מודפסת.\n"
                "   • אם המסמך מציג עבור מוצר נתון עמלת ספר + 'שיעור תגמול' (או כל עמודה אחרת ששמה שונה מ-'תוספת נפרעים') — אלה אינם בהכרח רכיבים שניתן לחבר. דווח אותם כשורות נפרדות ('עמלת ספר X%, שיעור תגמול Y%'), והוסף סה״כ **רק אם המסמך עצמו מדפיס שורת סה״כ**.\n"
                "   • אסור להמציא ערכים שלא מופיעים מילולית במסמך. אם אתה לא רואה את המספר בטקסט — אל תכתוב אותו.\n"
                "   • דוגמה אסורה (באג שקרה): הפניקס 'השתלות וטיפולים מיוחדים' — עמלת ספר 15%, שיעור תגמול 7.2%. **אסור לענות '25.4%' (15 + 10.4 מומצא)**. התשובה הנכונה: 'עמלת ספר 15% + שיעור תגמול 7.2%'.\n"
                "   • אם המשתמש שואל על 'אחוז נפרעים' או 'סה״כ' ואין סה״כ מודפס — אמור 'המסמך מציג רכיבים נפרדים: עמלת ספר X% ושיעור תגמול Y%. סה״כ מפורש אינו מודפס בהסכם זה'.\n\n"
